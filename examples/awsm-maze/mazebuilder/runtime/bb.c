...
extern int maze_erode(uint32_t iterations);

extern int maze_findend(size_t *x, size_t *y);

extern int maze_findstart(size_t *x, size_t *y);

extern int maze_generate_prim(size_t startx, size_t starty);

extern int maze_reduce(uint32_t iterations);
...
extern int32_t wasmf_maze_erode(uint32_t iterations);

extern int32_t wasmf_maze_findend(uint32_t x, uint32_t y);

extern int32_t wasmf_maze_findstart(uint32_t x, uint32_t y);

extern int32_t wasmf_maze_generate_prim(uint32_t startx, uint32_t starty);

extern int32_t wasmf_maze_reduce(uint32_t iterations);

int export_maze_findend(size_t *x, size_t *y) {
    return wasmf_maze_findend(from_ptr(x), from_ptr(y));
}

int export_maze_findstart(size_t *x, size_t *y) {
    return wasmf_maze_findstart(from_ptr(x), from_ptr(y));
}
...
const uint32_t __box_exportjumptable[] = {
    ...
    (uint32_t)__box_push,
    (uint32_t)__box_pop,
    (uint32_t)wasmf_maze_erode,
    (uint32_t)export_maze_findend,
    (uint32_t)export_maze_findstart,
    (uint32_t)wasmf_maze_generate_prim,
    (uint32_t)wasmf_maze_reduce,
};
