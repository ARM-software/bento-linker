...
//// mazebuilder state ////
struct __box_state __box_mazebuilder_state;
extern uint32_t __box_mazebuilder_jumptable[];

const struct __box_mpuregions __box_mazebuilder_mpuregions = {
    .control = 1,
    .count = 2,
    .regions = {
        {0x000f0000, 0x0200001f},
...
int maze_erode(uint32_t iterations) {
    if (!__box_mazebuilder_state.initialized) {
    ...
    extern int __box_import_maze_erode(uint32_t iterations);
    return __box_import_maze_erode(iterations);
}
...
int maze_findend(size_t *x, size_t *y) {
    ...
    extern int __box_import_maze_findend(size_t *x, size_t *y);
    return __box_import_maze_findend(x, y);
}
...
int maze_findstart(size_t *x, size_t *y) {
    ...
    extern int __box_import_maze_findstart(size_t *x, size_t *y);
    return __box_import_maze_findstart(x, y);
}
...
int maze_generate_prim(size_t startx, size_t starty) {
    ...
    extern int __box_import_maze_generate_prim(size_t startx, size_t starty);
    return __box_import_maze_generate_prim(startx, starty);
}
...
int maze_reduce(uint32_t iterations) {
    ...
    extern int __box_import_maze_reduce(uint32_t iterations);
    return __box_import_maze_reduce(iterations);
}
...
const uint32_t __box_mazebuilder_sys_jumptable[] = {
...
int __box_mazebuilder_init(void) {
    ...
    __box_mazebuilder_state.lr = 0xfffffffd; // TODO determine fp?
    __box_mazebuilder_state.sp = (void*)__box_mazebuilder_jumptable[0];
    ...
    // call box's init
    extern int __box_mazebuilder_postinit(void);
    err = __box_mazebuilder_postinit();
    if (err) {
        return err;
    }

    __box_mazebuilder_state.initialized = true;
    return 0;
}

int __box_mazebuilder_clobber(void) {
    __box_mazebuilder_state.initialized = false;
    return 0;
}
...
void *__box_mazebuilder_push(size_t size) {
    ...
    if (__box_mazebuilder_state.sp - size < (uint32_t*)&__box_mazebuilder_ram_start) {
        return NULL;
    }

    __box_mazebuilder_state.sp -= size;
    return __box_mazebuilder_state.sp;
}
...
void __box_mazebuilder_pop(size_t size) {
    ...
    assert(__box_mazebuilder_state.sp + size <= (uint32_t*)&__box_mazebuilder_ram_end);
    __box_mazebuilder_state.sp += size;
}
...
//// mazesolver state ////
struct __box_state __box_mazesolver_state;
extern uint32_t __box_mazesolver_jumptable[];

const struct __box_mpuregions __box_mazesolver_mpuregions = {
    .control = 1,
    .count = 2,
    .regions = {
...
int32_t maze_solve(size_t startx, size_t starty, size_t endx, size_t endy) {
    if (!__box_mazesolver_state.initialized) {
    ...
    extern int32_t __box_import_maze_solve(size_t startx, size_t starty, size_t endx, size_t endy);
    return __box_import_maze_solve(startx, starty, endx, endy);
}
...
const uint32_t __box_mazesolver_sys_jumptable[] = {
...
int __box_mazesolver_init(void) {
    ...
    // prepare the box's stack
    // must use PSP, otherwise boxes could overflow the ISR stack
    __box_mazesolver_state.lr = 0xfffffffd; // TODO determine fp?
    __box_mazesolver_state.sp = (void*)__box_mazesolver_jumptable[0];
    ...
    // call box's init
    extern int __box_mazesolver_postinit(void);
    err = __box_mazesolver_postinit();
    if (err) {
        return err;
    }

    __box_mazesolver_state.initialized = true;
    return 0;
}

int __box_mazesolver_clobber(void) {
    __box_mazesolver_state.initialized = false;
    return 0;
}
...
void *__box_mazesolver_push(size_t size) {
    ...
    if (__box_mazesolver_state.sp - size < (uint32_t*)&__box_mazesolver_ram_start) {
        return NULL;
    }

    __box_mazesolver_state.sp -= size;
    return __box_mazesolver_state.sp;
}
...
void __box_mazesolver_pop(size_t size) {
    ...
    assert(__box_mazesolver_state.sp + size <= (uint32_t*)&__box_mazesolver_ram_end);
    __box_mazesolver_state.sp += size;
}
...
struct __box_state *const __box_state[__BOX_COUNT+1] = {
    &__box_sys_state,
    &__box_mazebuilder_state,
    &__box_mazesolver_state,
};
...
const struct __box_mpuregions *const __box_mpuregions[__BOX_COUNT+1] = {
    ...
    &__box_mazebuilder_mpuregions,
    &__box_mazesolver_mpuregions,
};

const uint32_t *const __box_jumptables[__BOX_COUNT] = {
    __box_mazebuilder_jumptable,
    __box_mazesolver_jumptable,
};

const uint32_t *const __box_sys_jumptables[__BOX_COUNT] = {
    __box_mazebuilder_sys_jumptable,
    __box_mazesolver_sys_jumptable,
};
