...
void main(void) {
    ...
    printf("testing fib\n");
    uint32_t *fib_buffer = boxc_fib_alloc(10*sizeof(uint32_t));
    assert(fib_buffer);
    int res = boxc_fib(fib_buffer, 10*sizeof(uint32_t), 0, 1);
    printf("result: %d\n", res);
    printf("fib: [");
    for (int i = 0; i < 10; i++) {
    ...
    uint32_t *qsort_buffer = boxc_qsort_alloc(10*sizeof(uint32_t));
    assert(qsort_buffer);
    // "random"
    memcpy(qsort_buffer,
            (uint32_t[]){9,4,7,5,1,2,6,3,8,0},
            10*sizeof(uint32_t));
    res = boxc_qsort(qsort_buffer, 10);
    printf("result: %d\n", res);
    printf("qsort: [");
    for (int i = 0; i < 10; i++) {
