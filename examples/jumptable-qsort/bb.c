...
jmp_buf *__box_qsort_jmpbuf = NULL;
uint8_t *__box_qsort_datasp = NULL;
extern uint32_t __box_qsort_jumptable[];
#define __box_qsort_exportjumptable __box_qsort_jumptable

//// qsort exports ////

int32_t __box_qsort_postinit(const uint32_t *a0) {
    jmp_buf *pjmpbuf = __box_qsort_jmpbuf;
    jmp_buf jmpbuf;
    __box_qsort_jmpbuf = &jmpbuf;
    int err = setjmp(jmpbuf);
    if (err) {
        __box_qsort_jmpbuf = pjmpbuf;
        return err;
    }
    int32_t r0 = ((int32_t (*)(const uint32_t *a0))
            __box_qsort_exportjumptable[1])(a0);
    __box_qsort_jmpbuf = pjmpbuf;
    return r0;
}
...
int box_qsort(uint32_t *buffer, size_t size) {
    ...
    int r0 = ((int (*)(uint32_t *buffer, size_t size))
            __box_qsort_exportjumptable[2])(buffer, size);
    __box_qsort_jmpbuf = pjmpbuf;
    return r0;
}
...
__attribute__((noreturn))
void __box_qsort_abort(int err) {
    __box_qsort_initialized = false;
    if (__box_qsort_jmpbuf) {
        longjmp(*__box_qsort_jmpbuf, err);
    } else {
        __box_abort(err);
    }
}
...
const uint32_t __box_qsort_importjumptable[] = {
    (uint32_t)__box_qsort_abort,
...
int __box_qsort_init(void) {
    ...
    // call box's init
    err = __box_qsort_postinit(__box_qsort_importjumptable);
    if (err) {
        return err;
    }
...
void *__box_qsort_push(size_t size) {
    ...
    if (__box_qsort_datasp - size < &__box_qsort_ram_start) {
        return NULL;
    }

    __box_qsort_datasp -= size;
    return __box_qsort_datasp;
}
...
void __box_qsort_pop(size_t size) {
    ...
    assert(__box_qsort_datasp + size <= &__box_qsort_ram_end);
    __box_qsort_datasp += size;
}
