...
//// mandlebrot state ////
struct __box_state __box_mandlebrot_state;
extern uint32_t __box_mandlebrot_jumptable[];

const struct __box_mpuregions __box_mandlebrot_mpuregions = {
    .control = 1,
    .count = 2,
    .regions = {
...
int mandlebrot(size_t width, size_t height, uint32_t iterations) {
    if (!__box_mandlebrot_state.initialized) {
    ...
    extern int __box_import_mandlebrot(size_t width, size_t height, uint32_t iterations);
    return __box_import_mandlebrot(width, height, iterations);
}
...
const uint32_t __box_mandlebrot_sys_jumptable[] = {
    (uint32_t)__box_mandlebrot_write,
    (uint32_t)__box_mandlebrot_flush,
};
...
int __box_mandlebrot_init(void) {
    ...
    __box_mandlebrot_state.lr = 0xfffffffd; // TODO determine fp?
    __box_mandlebrot_state.sp = (void*)__box_mandlebrot_jumptable[0];
    ...
    // call box's init
    extern int __box_mandlebrot_postinit(void);
    err = __box_mandlebrot_postinit();
    if (err) {
        return err;
    }

    __box_mandlebrot_state.initialized = true;
    return 0;
}

int __box_mandlebrot_clobber(void) {
    __box_mandlebrot_state.initialized = false;
    return 0;
}

void *__box_mandlebrot_push(size_t size) {
    size = (size+3)/4;
    extern uint8_t __box_mandlebrot_ram_start;
    if (__box_mandlebrot_state.sp - size < (uint32_t*)&__box_mandlebrot_ram_start) {
        return NULL;
    }

    __box_mandlebrot_state.sp -= size;
    return __box_mandlebrot_state.sp;
}

void __box_mandlebrot_pop(size_t size) {
    size = (size+3)/4;
    __attribute__((unused))
    extern uint8_t __box_mandlebrot_ram_end;
    assert(__box_mandlebrot_state.sp + size <= (uint32_t*)&__box_mandlebrot_ram_end);
    __box_mandlebrot_state.sp += size;
}
