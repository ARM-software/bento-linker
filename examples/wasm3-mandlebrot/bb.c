...
IM3Runtime __box_mandlebrot_runtime;
IM3Module __box_mandlebrot_module;
uint32_t __box_mandlebrot_datasp;

__attribute__((unused))
static uint32_t __box_mandlebrot_fromptr(const void *ptr) {
    return (uint32_t)((const uint8_t*)ptr
        - m3MemData(__box_mandlebrot_runtime->memory.mallocated));
}

__attribute__((unused))
static void *__box_mandlebrot_toptr(uint32_t ptr) {
    return m3MemData(__box_mandlebrot_runtime->memory.mallocated) + ptr;
}
...
void *__box_mandlebrot_push(size_t size) {
    ...
    return __box_mandlebrot_toptr(psp);
}
...
m3ApiRawFunction(__box_mandlebrot_import___box_mandlebrot_abort) {
    m3ApiGetArg(int, err);
    __box_mandlebrot_runtime->exit_code = err;
...
m3ApiRawFunction(__box_mandlebrot_import___box_mandlebrot_write) {
    m3ApiReturnType(ssize_t);
    m3ApiGetArg(int32_t, a0);
    m3ApiGetArgMem(const void*, a1);
    m3ApiGetArg(size_t, a2);
    ssize_t r0 = __box_mandlebrot_write(a0, a1, a2);
    m3ApiReturn(r0);
}

m3ApiRawFunction(__box_mandlebrot_import___box_mandlebrot_flush) {
    m3ApiReturnType(int);
    m3ApiGetArg(int32_t, a0);
    int r0 = __box_mandlebrot_flush(a0);
    m3ApiReturn(r0);
}
...
int mandlebrot(size_t width, size_t height, uint32_t iterations) {
    ...
    *(uint32_t*)&stack[2] = iterations;
    m3StackCheckInit();
    res = (M3Result)Call(
            f->compiled,
            (m3stack_t)stack,
            __box_mandlebrot_runtime->memory.mallocated,
            d_m3OpDefaultArgs);
    if (res) {
        if (res == m3Err_trapExit) {
            return __box_mandlebrot_runtime->exit_code;
        }
...
int __box_mandlebrot_init(void) {
    ...
    __box_mandlebrot_runtime = m3_NewRuntime(
