...
//// lfsbox state ////
struct __box_state __box_lfsbox_state;
extern uint32_t __box_lfsbox_jumptable[];

const struct __box_mpuregions __box_lfsbox_mpuregions = {
    .control = 1,
    .count = 2,
    .regions = {
        {0x000f8000, 0x0200001d},
        {0x2003c000, 0x1300001b},
    },
};
...
int lfsbox_file_close(int32_t fd) {
    if (!__box_lfsbox_state.initialized) {
    ...
    extern int __box_import_lfsbox_file_close(int32_t fd);
    return __box_import_lfsbox_file_close(fd);
}
...
int32_t lfsbox_file_open(const char *path, uint32_t flags) {
    ...
    extern int32_t __box_import_lfsbox_file_open(const char *path, uint32_t flags);
    return __box_import_lfsbox_file_open(path, flags);
}
...
ssize_t lfsbox_file_read(int32_t fd, void *buffer, size_t size) {
    ...
    extern ssize_t __box_import_lfsbox_file_read(int32_t fd, void *buffer, size_t size);
    return __box_import_lfsbox_file_read(fd, buffer, size);
}
...
int32_t lfsbox_file_seek(int32_t fd, int32_t off, uint32_t whence) {
    ...
    extern int32_t __box_import_lfsbox_file_seek(int32_t fd, int32_t off, uint32_t whence);
    return __box_import_lfsbox_file_seek(fd, off, whence);
}
...
ssize_t lfsbox_file_write(int32_t fd, const void *buffer, size_t size) {
    ...
    extern ssize_t __box_import_lfsbox_file_write(int32_t fd, const void *buffer, size_t size);
    return __box_import_lfsbox_file_write(fd, buffer, size);
}
...
int lfsbox_format(void) {
    ...
    extern int __box_import_lfsbox_format(void);
    return __box_import_lfsbox_format();
}
...
int lfsbox_mount(void) {
    ...
    extern int __box_import_lfsbox_mount(void);
    return __box_import_lfsbox_mount();
}
...
int lfsbox_rename(const char *oldpath, const char *newpath) {
    ...
    extern int __box_import_lfsbox_rename(const char *oldpath, const char *newpath);
    return __box_import_lfsbox_rename(oldpath, newpath);
}
...
int lfsbox_unmount(void) {
    ...
    extern int __box_import_lfsbox_unmount(void);
    return __box_import_lfsbox_unmount();
}
...
const uint32_t __box_lfsbox_sys_jumptable[] = {
    (uint32_t)__box_lfsbox_write,
    (uint32_t)__box_lfsbox_flush,
    (uint32_t)bd_block_count,
...
int __box_lfsbox_init(void) {
    ...
    __box_lfsbox_state.lr = 0xfffffffd; // TODO determine fp?
    __box_lfsbox_state.sp = (void*)__box_lfsbox_jumptable[0];
    ...
    // call box's init
    extern int __box_lfsbox_postinit(void);
    err = __box_lfsbox_postinit();
    if (err) {
        return err;
    }

    __box_lfsbox_state.initialized = true;
    return 0;
}

int __box_lfsbox_clobber(void) {
    __box_lfsbox_state.initialized = false;
    return 0;
}

void *__box_lfsbox_push(size_t size) {
    size = (size+3)/4;
    extern uint8_t __box_lfsbox_ram_start;
    if (__box_lfsbox_state.sp - size < (uint32_t*)&__box_lfsbox_ram_start) {
        return NULL;
    }

    __box_lfsbox_state.sp -= size;
    return __box_lfsbox_state.sp;
}

void __box_lfsbox_pop(size_t size) {
    size = (size+3)/4;
    __attribute__((unused))
    extern uint8_t __box_lfsbox_ram_end;
    assert(__box_lfsbox_state.sp + size <= (uint32_t*)&__box_lfsbox_ram_end);
    __box_lfsbox_state.sp += size;
}
