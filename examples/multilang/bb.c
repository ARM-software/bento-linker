...
// Initialize box boxc. Resets the box to its initial state if already
// initialized.
...
static int __box_boxc_load(void) {
...
int __box_boxc_init(void) {
    ...
    __box_boxc_state.initialized = true;
    return 0;
}
...
extern uint32_t __box_boxrust_jumptable[];

const struct __box_mpuregions __box_boxrust_mpuregions = {
    .control = 1,
    .count = 2,
    .regions = {
        {0x000f0000, 0x0200001d},
...
int32_t boxrust_add2(int32_t a0, int32_t a1) {
    if (!__box_boxrust_state.initialized) {
        int err = __box_boxrust_init();
        if (err) {
            return err;
        }
    }

    extern int32_t __box_import_boxrust_add2(int32_t a0, int32_t a1);
    return __box_import_boxrust_add2(a0, a1);
}
...
int boxrust_fib(uint32_t *buffer, size_t size, uint32_t a, uint32_t b) {
    ...
    extern int __box_import_boxrust_fib(uint32_t *buffer, size_t size, uint32_t a, uint32_t b);
    return __box_import_boxrust_fib(buffer, size, a, b);
}
...
void* boxrust_fib_alloc(size_t size) {
    ...
    extern void* __box_import_boxrust_fib_alloc(size_t size);
    return __box_import_boxrust_fib_alloc(size);
}
...
int boxrust_fib_next(uint32_t *next, uint32_t a, uint32_t b) {
    ...
    extern int __box_import_boxrust_fib_next(uint32_t *next, uint32_t a, uint32_t b);
    return __box_import_boxrust_fib_next(next, a, b);
}
...
int boxrust_hello(void) {
    ...
    extern int __box_import_boxrust_hello(void);
    return __box_import_boxrust_hello();
}
...
int boxrust_qsort(uint32_t *buffer, size_t size) {
    ...
    extern int __box_import_boxrust_qsort(uint32_t *buffer, size_t size);
    return __box_import_boxrust_qsort(buffer, size);
}
...
void* boxrust_qsort_alloc(size_t size) {
    ...
    extern void* __box_import_boxrust_qsort_alloc(size_t size);
    return __box_import_boxrust_qsort_alloc(size);
}
...
ssize_t boxrust_qsort_partition(uint32_t *buffer, size_t size, uint32_t pivot) {
    ...
    extern ssize_t __box_import_boxrust_qsort_partition(uint32_t *buffer, size_t size, uint32_t pivot);
    return __box_import_boxrust_qsort_partition(buffer, size, pivot);
}

//// boxrust imports ////

// redirect __box_boxrust_write -> __box_write
#define __box_boxrust_write __box_write

// redirect __box_boxrust_flush -> __box_flush
#define __box_boxrust_flush __box_flush

const uint32_t __box_boxrust_sys_jumptable[] = {
    (uint32_t)__box_boxrust_write,
    (uint32_t)__box_boxrust_flush,
};
...
int __box_boxrust_init(void) {
    ...
    __box_boxrust_state.sp = (void*)__box_boxrust_jumptable[0];
    ...
    // call box's init
    extern int __box_boxrust_postinit(void);
    err = __box_boxrust_postinit();
    if (err) {
        return err;
    }
...
void *__box_boxrust_push(size_t size) {
    size = (size+3)/4;
    extern uint8_t __box_boxrust_ram_start;
    if (__box_boxrust_state.sp - size < (uint32_t*)&__box_boxrust_ram_start) {
        return NULL;
    }

    __box_boxrust_state.sp -= size;
    return __box_boxrust_state.sp;
}

void __box_boxrust_pop(size_t size) {
    size = (size+3)/4;
    __attribute__((unused))
    extern uint8_t __box_boxrust_ram_end;
    assert(__box_boxrust_state.sp + size <= (uint32_t*)&__box_boxrust_ram_end);
    __box_boxrust_state.sp += size;
}
