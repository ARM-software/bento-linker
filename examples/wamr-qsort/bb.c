...
wasm_module_t __box_qsort_module;
wasm_module_inst_t __box_qsort_module_inst;
wasm_exec_env_t __box_qsort_exec_env;
int __box_qsort_err;

// default __box_abort implementation
void __box_qsort_import___box_qsort_abort(
...
ssize_t __box_qsort_import___box_qsort_write(wasm_exec_env_t env, int32_t a0, const void *a1, size_t a2) {
    return __box_qsort_write(a0, a1, a2);
}

int __box_qsort_import___box_qsort_flush(wasm_exec_env_t env, int32_t a0) {
    return __box_qsort_flush(a0);
}

const NativeSymbol __box_qsort_native_symbols[] = {
    {
        "__box_abort",
...
int __box_qsort_init(void) {
    ...
    if (!__box_qsort_module) {
        bool success = wasm_runtime_register_natives(
            "env",
            (NativeSymbol*)__box_qsort_native_symbols,
            sizeof(__box_qsort_native_symbols) / sizeof(NativeSymbol));
        if (!success) {
            return -EGENERAL;
        }
    }
    ...
    __box_qsort_module = wasm_runtime_load(
...
int __box_qsort_clobber(void) {
    if (__box_qsort_initialized) {
        wasm_runtime_destroy_exec_env(__box_qsort_exec_env);
        wasm_runtime_deinstantiate(__box_qsort_module_inst);
        wasm_runtime_unload(__box_qsort_module);
    }
