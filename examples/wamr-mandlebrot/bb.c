...
int mandlebrot(size_t width, size_t height, uint32_t iterations);
...
// Initialize box mandlebrot. Resets the box to its initial state if already
// initialized.
int __box_mandlebrot_init(void);

// Mark the box mandlebrot as needing to be reinitialized.
int __box_mandlebrot_clobber(void);

// Allocate size bytes on the box's data stack. May return NULL if a stack
// overflow would occur.
void *__box_mandlebrot_push(size_t size);

// Deallocate size bytes on the box's data stack.
void __box_mandlebrot_pop(size_t size);
...
static int __box_mandlebrot_load(void) {
    // default loader does nothing
    return 0;
}

//// mandlebrot state ////
bool __box_mandlebrot_initialized = false;
uint32_t __box_mandlebrot_datasp;
wasm_module_t __box_mandlebrot_module;
wasm_module_inst_t __box_mandlebrot_module_inst;
wasm_exec_env_t __box_mandlebrot_exec_env;
int __box_mandlebrot_err;

// default __box_abort implementation
void __box_mandlebrot_import___box_mandlebrot_abort(
...
// redirect __box_mandlebrot_write -> __box_write
#define __box_mandlebrot_write __box_write

// redirect __box_mandlebrot_flush -> __box_flush
#define __box_mandlebrot_flush __box_flush

//// mandlebrot imports ////

ssize_t __box_mandlebrot_import___box_mandlebrot_write(wasm_exec_env_t env, int32_t a0, const void *a1, size_t a2) {
    return __box_mandlebrot_write(a0, a1, a2);
}

int __box_mandlebrot_import___box_mandlebrot_flush(wasm_exec_env_t env, int32_t a0) {
    return __box_mandlebrot_flush(a0);
}

const NativeSymbol __box_mandlebrot_native_symbols[] = {
    {
        "__box_abort",
        __box_mandlebrot_import___box_mandlebrot_abort,
        "(i)",
        &__box_mandlebrot_err,
    },
    {
        "__box_write",
        __box_mandlebrot_import___box_mandlebrot_write,
        "(i*i)i",
        NULL,
    },
    {
        "__box_flush",
        __box_mandlebrot_import___box_mandlebrot_flush,
        "(i)i",
        NULL,
    },
};
...
int mandlebrot(size_t width, size_t height, uint32_t iterations) {
    ...
    __box_mandlebrot_err = 0;

    __attribute__((aligned(4)))
    uint8_t frame[4*3];
    *(size_t*)&frame[4*0] = width;
    *(size_t*)&frame[4*1] = height;
    *(uint32_t*)&frame[4*2] = iterations;
    bool res = wasm_runtime_call_wasm(
        ...
        (uint32_t*)frame);
    if (!res) {
        if (__box_mandlebrot_err) {
            wasm_runtime_set_exception(
                __box_mandlebrot_module_inst,
                NULL);
            return __box_mandlebrot_err;
        }
        return -EFAULT;
    }
...
int __box_mandlebrot_init(void) {
    ...
    if (!__box_mandlebrot_module) {
        bool success = wasm_runtime_register_natives(
            "env",
            (NativeSymbol*)__box_mandlebrot_native_symbols,
            sizeof(__box_mandlebrot_native_symbols) / sizeof(NativeSymbol));
        if (!success) {
            return -EGENERAL;
        }
    }
    ...
    __box_mandlebrot_module = wasm_runtime_load(
    ...
    __box_mandlebrot_module_inst = wasm_runtime_instantiate(
        __box_mandlebrot_module,
        32768,
        0,
        NULL, 0);
    if (!__box_mandlebrot_module_inst) {
        return -ENOEXEC;
    }

    __box_mandlebrot_exec_env = wasm_runtime_create_exec_env(
        __box_mandlebrot_module_inst,
        32768);
    if (!__box_mandlebrot_exec_env) {
        return -ENOEXEC;
    }
...
int __box_mandlebrot_clobber(void) {
    if (__box_mandlebrot_initialized) {
        wasm_runtime_destroy_exec_env(__box_mandlebrot_exec_env);
        wasm_runtime_deinstantiate(__box_mandlebrot_module_inst);
        wasm_runtime_unload(__box_mandlebrot_module);
    }
...
void *__box_mandlebrot_push(size_t size) {
    ...
    uint32_t psp = __box_mandlebrot_datasp;
    if (psp + size > 32768) {
        return NULL;
    }

    __box_mandlebrot_datasp = psp + size;
    return wasm_runtime_addr_app_to_native(
            __box_mandlebrot_module_inst, psp);
}

void __box_mandlebrot_pop(size_t size) {
    assert(__box_mandlebrot_datasp - size >= 0);
    __box_mandlebrot_datasp -= size;
}
