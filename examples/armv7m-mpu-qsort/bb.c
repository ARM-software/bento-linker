...
//// qsort state ////
struct __box_state __box_qsort_state;
extern uint32_t __box_qsort_jumptable[];

const struct __box_mpuregions __box_qsort_mpuregions = {
    .control = 1,
    .count = 2,
    .regions = {
...
int box_qsort(uint32_t *buffer, size_t size) {
    if (!__box_qsort_state.initialized) {
    ...
    extern int __box_import_box_qsort(uint32_t *buffer, size_t size);
    return __box_import_box_qsort(buffer, size);
}
...
const uint32_t __box_qsort_sys_jumptable[] = {
    (uint32_t)__box_qsort_write,
    (uint32_t)__box_qsort_flush,
};
...
int __box_qsort_init(void) {
    ...
    __box_qsort_state.lr = 0xfffffffd; // TODO determine fp?
    __box_qsort_state.sp = (void*)__box_qsort_jumptable[0];
    ...
    // call box's init
    extern int __box_qsort_postinit(void);
    err = __box_qsort_postinit();
    if (err) {
        return err;
    }

    __box_qsort_state.initialized = true;
    return 0;
}

int __box_qsort_clobber(void) {
    __box_qsort_state.initialized = false;
    return 0;
}

void *__box_qsort_push(size_t size) {
    size = (size+3)/4;
    extern uint8_t __box_qsort_ram_start;
    if (__box_qsort_state.sp - size < (uint32_t*)&__box_qsort_ram_start) {
        return NULL;
    }

    __box_qsort_state.sp -= size;
    return __box_qsort_state.sp;
}

void __box_qsort_pop(size_t size) {
    size = (size+3)/4;
    __attribute__((unused))
    extern uint8_t __box_qsort_ram_end;
    assert(__box_qsort_state.sp + size <= (uint32_t*)&__box_qsort_ram_end);
    __box_qsort_state.sp += size;
}
